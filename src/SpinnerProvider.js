...
class SpinnerProvider extends PureComponent {
    state = {
        open: false,
        close: false,
        message: null,
    ...
    processQueue = () => {
        if (this.props.spinner) {
            const { message, action, handleAction, close, variant, direction } = this.props.spinner.options
            if (variant) {
                this.setState({ variant });
            }
            this.setState({ open: true, message, action, close, direction, handleAction })
            this.props.dismiss(this.props.spinner.id)
        }
    }
...
SpinnerProvider.childContextTypes = {
...
SpinnerProvider = withStyles(styles)(SpinnerProvider);

export default connect(
    state => ({
        spinner: state.spinner.queue[0] || null
    }),
    dispatch => ({
